...
export default class FileUploader {
	constructor({
		...
		if (!wrapper) {
			this.make_dialog(dialog_title, frm);
		} else {
		...
		var html = this.uploader.$el;
		html.children[0].appendChild(this.wrapper);
	...
	upload_files(values) {
		...
		return this.uploader.upload_files(values);
	}

	make_dialog(title, frm) {
		var modeOfPaymentsOptions = null;
		this.dialog = new frappe.ui.Dialog({
			title: title || __("Upload"),
			fields: frm?.custom_s42_upload_fields || [],
			primary_action_label: __("Upload"),
			primary_action: (values) => this.upload_files(values),
