			...
			<div v-if="!is_dragging">
				...
				<div class="mt-2 text-center">
					<button class="btn btn-file-upload" @click="browse_files">
						...
						<div id="s" class="mt-1">{{ __('My Device') }}</div>
					</button>
					<input
						type="file"
						class="hidden"
...
<script>
import FilePreview from 'frappe/public/js/frappe/file_uploader/FilePreview.vue';
import FileBrowser from 'frappe/public/js/frappe/file_uploader/FileBrowser.vue';
import WebLink from 'frappe/public/js/frappe/file_uploader/WebLink.vue';
import GoogleDrivePicker from 'frappe/public/js/integrations/google_drive_picker';
import ImageCropper from 'frappe/public/js/frappe/file_uploader/ImageCropper.vue';
	...
	methods: {
		...
		upload_files(values) {
			if (this.show_file_browser) {
				return this.upload_via_file_browser(values);
			}
			...
			return frappe.run_serially(
				this.files.map(
					(file, i) =>
						() => this.upload_file(file,values, i)
				)
			);
		},
		upload_via_file_browser(values) {
		...
		upload_file(file, i) {
			...
			return new Promise((resolve, reject) => {
				...
				xhr.open('POST', '/api/method/frappe_s3_attachment.overrides.custom_file_upload.custom_file_uploader', true);
				...
				if (this.doctype && this.docname) {
					...
					form_data.append('values', file.values);
				}
